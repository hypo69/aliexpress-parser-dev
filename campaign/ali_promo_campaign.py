...
from src.utils import j_dumps, j_loads_ns, j_loads
from src.utils.convertor import csv2dict
...
class AliPromoCampaign:
    ...
    def __init__(
        ...
        self.campaign_ai_file_name = f"{language}_{currency}_{model}_{gs.now}.json"
        ...
        #self.model = OpenAIModel(system_instruction=system_instruction)  if model.lower() == 'openai' else GoolgeGenerativeAI(system_instruction=system_instruction)
    ...
    def process_campaign_ai_properties(self):
        """Установка значений `title`, `description` для заголовка рекламной кампании"""
        ...
    ...
    def process_ai_category(self, category_name: Optional[str] = None):
        ...
        j_dumps(campaign_ai, self.base_path / "ai" / self.campaign_ai_file_name)
        return
    ...
    async def generate_html(self, campaign_name:str, category_path: str | Path, products_list: list[SimpleNamespace] | SimpleNamespace):
        ...
        for product in products_list:
            ...
            <img src="{product.image_local_saved_path}" alt="{html.escape(product.product_title)}" class="product-image">
